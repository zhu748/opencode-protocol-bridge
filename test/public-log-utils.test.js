...
const logs = [
  ...
  { time: 'invalid', requestId: 'local-upstream', provider: 'zen', status: 502, protocol: 'claude → chat', requestedReasoningEffort: 'max', reasoningEffort: 'high', bridgeWebSearchCalls: 37, responseDegradations: 'claude_iterations', errorCode: 'upstream_dns_error', error: '连接失败' }
];
...
test('CSV 导出限定元数据字段并防止公式注入', () => {
  const csv = requestLogsToCsv([{
    ...
    requestedReasoningEffort: 'max', reasoningEffort: 'high', bridgeWebSearchCalls: 37, responseDegradations: 'claude_cache_creation_ttl', cacheCreation5mInputTokens: 3, cacheCreation1hInputTokens: 4,
