...
test('同一凭据对象复用身份哈希且字段变更时自动失效', () => {
  ...
  assert.equal(health.snapshot('go', [credential])[0].state, 'cooldown');
});
...
test('大 Key 池故障切换原地跳过已尝试项且不计算其密钥指纹', () => {
  ...
  const cooling = new CredentialHealthTracker({ now: () => 1000, rateLimitCooldownMs: 30_000 });
  cooling.recordResponse('go', largePool[0], 429, '10');
  cooling.recordResponse('go', largePool[1], 429, '20');
  assert.deepEqual(cooling.select('go', largePool.slice(0, 2), new Set([largePool[0].credentialId])), {
    credential: null, reason: 'cooldown', retryAfterMs: 20_000
  });
});
