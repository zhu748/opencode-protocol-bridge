...
- 工具调用、工具结果、并行工具开关、文本消息、图片精度字段，以及 Claude Documents 与 Responses 文件块转换；Codex Responses `namespace` 工具可跨协议桥接到 Chat/Claude；Claude Code 的 typed `web_search_YYYYMMDD`、内置客户端 `WebSearch` 和 Codex Responses `web_search` 可在 Chat 路由通过本地 Exa/Parallel MCP 工具循环执行
...
`CONFIG_ENCRYPTION_KEY` 由 Render 自动生成，`HOST=0.0.0.0`、`OPENCODE_BRIDGE_TRUST_PROXY=true` 和 `OPENCODE_BRIDGE_REQUIRE_ENV_BOOTSTRAP=true` 已在 Blueprint 中设置，`PORT` 由 Render 自动注入。最后一项会在这两项必填 Secret 缺失时拒绝启动，避免首次公网访问者抢先初始化控制台。可信代理开关使登录限速按 Render 提供的真实客户端地址隔离，普通自托管部署默认不信任转发头。批量 `*_KEYS` 非空时优先于编号变量，任一环境变量 Key 池又优先于管理面板保存的 Key 池；Key 按请求轮询，每把使用对应位置或同编号代理。面板 Key 池同样最多支持 32 把，每项可单独命名、测试和设置代理；旧版保存的单 Key 会在首次编辑时自动迁移。401/403/429 会在同一请求内安全切换到下一把未尝试的 Key，响应头 `x-opencode-key-attempts` 会在发生切换时给出尝试次数。网络错误或 5xx 不会自动重放推理请求，以避免重复计费或重复工具调用；幂等的模型发现请求在建连、响应头或正文阶段发生网络错误时会安全尝试下一把 Key。失败状态只用于健康排序和面板展示，不会封锁任何已配置 Key；当全部 Key 都有失败记录时仍继续轮询。非流式 2xx 只有在正文完整读取、JSON 解析和响应结构校验全部完成后才标记为成功；正文中途断开计为网络失败，客户端主动取消则保持凭据中性。健康状态只保存在当前进程内，替换同一槽位的 Key 或代理不会继承旧状态。除保活运行时覆盖外，环境变量引导仅在配置文件中还没有管理密码时执行，不会覆盖已存在的持久化配置；保活环境变量则在每次启动时生效并优先于面板值。

Render 免费 Web Service 的文件系统是临时的，闲置 15 分钟后会休眠，重启或重新部署可能丢失管理面板写入的本地配置。因此免费部署应把长期使用的密码、令牌、Key 和代理保存为 Render Secret；实例恢复时项目会从这些变量重新生成加密配置。面板中临时修改的模型路由、替换规则和客户端列表也会在实例文件系统重置后恢复默认；请求日志、独立统计文件、Key 健康状态，以及最近 Claude system 快照同样无法跨文件系统重置保留。普通的进程停止再启动只要 `data` 目录还在，统计就会从 `data/request-stats.json` 恢复。需要跨 Render 重新部署永久保留时，应升级到支持 Persistent Disk 的付费实例并将磁盘挂载到 `/opt/render/project/src/data`。详见 [Render 免费实例限制](https://render.com/docs/free) 与 [Persistent Disks](https://render.com/docs/disks)。
...
- 成功 SSE 使用独立的双层流空闲超时：原始连接连续无字节会触发 `upstream_stream_idle_timeout`；上游即使持续发送注释、空帧或代理 keep-alive，只要连续没有可解析的有效 SSE JSON 事件，也会触发 `upstream_stream_event_idle_timeout`。合法 reasoning、正文、工具、usage 或终态事件会重新计时，因此正常持续输出的长任务没有总时长限制。超时会取消上游正文、发送客户端协议可识别的错误帧、写入 504 日志并参与 Key 熔断。该设置默认 300000 毫秒，可在管理面板设为 1000–3600000 毫秒，或设为 `0` 禁用。
...
- 流式请求只有在完整结束后才会把对应 Key 记为健康；包括正常关闭连接但缺少完成事件的截断流，都会写入失败日志并返回错误帧。客户端主动断开记为内部状态 499，但不会惩罚 Key。Key 的连续失败只用于健康排序和展示，不会阻断后续请求。同协议流的用量观察与字节透传位于同一条背压链路中，不会因统计分支提前读取而在慢客户端一侧无限积压。
...
管理面板可以设置 1 秒至 10 分钟的上游请求超时，默认 120 秒。非流式请求、模型发现和流式请求的非 2xx 错误正文会把它应用到完整上游正文；非流式/模型发现正文和流式错误正文读取结束、失败或因 Key 切换被丢弃后会立即释放对应定时器。成功的流式推理只用它限制建立连接及等待响应头。收到成功 SSE 响应头后不再设置总时长上限，改由默认 5 分钟、同时检查原始字节与有效 SSE 事件的独立流空闲超时识别停滞连接；客户端断开会记录为 499 `client_closed`、立即取消上游且不计入失败率或 Key 熔断。上游 JSON 与 SSE 均按 UTF-8 严格解码；非法字节会得到明确的 `upstream_invalid_utf8` 错误，不会被静默替换成 `�` 后继续转换。
...
- OpenAI Responses 的 `/responses/compact` 不会被伪装成 Chat/Claude/Gemini 的“等价压缩”：只有路由协议为 `responses` 时才会把请求原样发往上游。OpenCode 当前公开的 Zen/Go 模型表只承诺各模型的 `/responses`、`/messages` 或 `/chat/completions` 主端点，没有单独承诺 `/responses/compact`；因此该入口面向实际实现此扩展的上游，若 OpenCode 未开放会保留其规范化错误。Codex 会在后续请求中重放 `type: "compaction"` 与 `type: "context_compaction"` 历史项，两者的 `id` 均为可选，后者的 `encrypted_content` 也可选；Bridge 按官方结构校验并支持跨 Claude/Chat/Gemini 的可逆状态封装。客户端直接把这些 Responses 压缩状态发往其它协议时，不透明内容不会伪装成目标模型状态，并通过 `x-opencode-input-degradations: responses_compaction_state` 明确标记。
- Codex 多代理协作会把子代理消息作为独立的 Responses `agent_message` 历史项重放。跨到 Chat/Claude/Gemini 时，Bridge 会严格校验 `author`、`recipient` 和内容块，把完全可读的 `input_text` 按官方用户轮次边界转成带参与者标识的 user 消息，并标记 `responses_agent_message_to_user`；同协议 Responses 保持原样。包含 `encrypted_content` 的代理消息依赖 Responses 服务端解密，不能伪装成普通文本，因此会在访问上游前明确提示改用 Responses 路由。
...
- 本地 Web Search 设置同时服务 Claude Code → Chat 与 Codex Responses → Chat。Codex 搜索调用会在每次真实 MCP 执行后生成 `ws_` 输出项；若最终模型漏写来源，Bridge 会补齐最多 8 个去重链接并生成可校验的 URL 引用索引，因此 CLI 展示的搜索次数可以与管理日志 `bridgeWebSearchCalls` 交叉核对。模型同轮调用本地搜索与 Codex 客户端工具时，Bridge 会先返回已完成的 `web_search_call` 和仍需客户端执行的 `function_call`，待全部客户端工具结果回传后再恢复搜索结果继续推理；待续轮状态按客户端与模型隔离，最多保留 10 分钟、256 项和 32 MiB，避免长期会话造成无界内存增长。正常续轮会优先恢复完整搜索结果；若进程重启或状态过期，历史 `web_search_call` 会退化为包含动作、状态和查询的可读摘要，保留上下文语义并避免再次触发不支持类型错误。
