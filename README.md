...
- 非流式上游响应、错误正文和模型列表均有独立体积上限；模型发现成功响应会校验 `data` 数组、模型数量和 ID，错误响应统一转换为安全 JSON，不会把上游 HTML/代理错误页直接透传；损坏 JSON 会返回明确的 502 并写入元数据日志
...
- 入站 JSON 使用严格 UTF-8 解码并按用途限制体积：普通管理请求 64 KiB、完整配置 2 MiB、提示词预览约 2.1 MiB、推理请求 10 MiB。模型发现也绑定客户端取消信号，客户端断开后会立即取消上游读取且不会惩罚对应 Key。
