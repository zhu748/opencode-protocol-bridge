...
- Responses 历史项的 `id/status/phase` 会在跨协议前严格校验；由于 Chat/Claude/Gemini 没有等价的项级元数据字段，`id/status` 通过 `responses_item_metadata` 标记，Codex 历史 assistant 消息的 `phase: commentary|final_answer` 则通过 `responses_item_phase` 标记后降级为普通 assistant 历史。消息正文、工具 `call_id`、工具名称、参数和历史顺序仍完整保留；非法状态或 phase 不会被静默接受。这样 Codex 的完整历史可路由到 Chat 上游，而不会因仅用于 UI 通道区分的 phase 返回 400。
