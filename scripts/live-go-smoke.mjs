...
Object.assign(env, {
  ...
  OPENCODE_BRIDGE_ADMIN_PASSWORD: `Admin${randomBytes(12).toString('hex')}`,
...
try {
  ...
  const responses = await jsonRequest(`${base}/responses`, {
    method: 'POST', headers,
    body: JSON.stringify({ model, max_output_tokens: 24, input: 'Reply with exactly: OK' })
  });
  ...
  const streamResponse = await fetch(`${base}/chat/completions`, {
    method: 'POST', headers,
    body: JSON.stringify({ model, stream: true, stream_options: { include_usage: true }, max_tokens: 24, messages: [{ role: 'user', content: 'Reply with exactly: STREAM_OK' }] }),
    signal: AbortSignal.timeout(60_000)
  });
  const streamText = await streamResponse.text();
  if (!streamResponse.ok) throw new Error(`Chat 流式请求返回 HTTP ${streamResponse.status}: ${streamText.slice(0, 500)}`);
  if (!streamResponse.headers.get('content-type')?.includes('text/event-stream') || !streamText.includes('data: [DONE]')) throw new Error('Chat 流式响应缺少标准 SSE 结束事件');
  ...
  console.log(JSON.stringify({
    ...
    checks: {
      models: true,
      responses: { status: responses.status, inputTokens: responses.usage?.input_tokens ?? null, outputTokens: responses.usage?.output_tokens ?? null },
      claudeTool: { stopReason: claude.stop_reason, tool: toolBlock.name, input: toolBlock.input },
      chatStream: { contentType: streamResponse.headers.get('content-type'), done: true }
    }
  }, null, 2));
} finally {
  if (child.exitCode === null) child.kill();
  await Promise.race([once(child, 'exit'), new Promise((resolveWait) => setTimeout(resolveWait, 2000))]).catch(() => {});
