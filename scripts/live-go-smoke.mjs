...
try {
  ...
  const responsesStreamResponse = await fetch(`${base}/responses`, {
    method: 'POST', headers,
  ...
  const parsedResponsesStream = parseResponsesStream(responsesStreamText);
  ...
  const claudeToolResult = await jsonRequest(`${base}/messages`, {
    method: 'POST', headers: { ...headers, 'anthropic-version': '2023-06-01' },
    body: JSON.stringify({
      model, max_tokens: 48,
      messages: [
  ...
  if (stats.summary?.requests !== 5 || stats.summary?.usageRequests !== 5 || goStats?.requests !== 5) throw new Error('管理面板统计未覆盖五次在线协议请求');
  ...
  console.log(JSON.stringify({
    ...
    model,
    checks: {
      models: skipModelDiscovery ? 'skipped' : true,
