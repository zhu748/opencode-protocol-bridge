...
export function filterRequestLogs(items, { query = '', provider = 'all', status = 'all', timeRange = 'all', now = Date.now() } = {}) {
  ...
  return (Array.isArray(items) ? items : []).filter((item) => {
    ...
    return [item.requestId, item.upstreamRequestId, item.clientName, item.model, item.upstreamModel, item.provider, item.protocol, item.bridgeWebSearchCalls, item.responseDegradations, item.credentialId, item.credentialLabel, item.credentialAttempts, item.retryAfter, item.errorCode, item.error]
...
export function requestLogsToCsv(items) {
  const columns = [
    ...
    ['协议转换', 'protocol'], ['本地 Web Search 次数', 'bridgeWebSearchCalls'], ['响应元数据降级', 'responseDegradations'], ['状态', 'status'], ['输入 Token', 'inputTokens'], ['输出 Token', 'outputTokens'],
